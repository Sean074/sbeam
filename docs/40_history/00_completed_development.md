...
| [`07_maneuver_transient.md`](07_maneuver_transient.md) | Transient maneuver loads (Phase G0) | Increment 1 — ZAERO MLOADS card set, Level-1 quasi-steady, open-loop, restrained l-set Newmark-β; Step 59 — shared `reduce_to_aset` a-set reduction (`assembly/reduction.py`) + `Sol103Result` a-set eigendata retention; Step 60 — `MASSSET` payload / mass-case capability for static SOL 144. Future Steps 61–63 (modal basis, modal transient solver, free-flight) land here |
