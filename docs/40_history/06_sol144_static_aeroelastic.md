...
combination no HA144A or cessna deck covered. Before this, a corrected AIC had never
driven a trim anywhere in the repo (the cessna correction ladder stopped at SOL 101), and
no deck paired `MLOADS` with `MASSSET`.

**Deliverables.**
- `sample/cessna210_flagship_bulk.bdf` — the whole bulk model — plus three thin
  case-control drivers over it via whole-bulk `INCLUDE`: `_trim` (SC1 1g cruise + SC2 2.5g
  pull-up), `_massset` (one TRIM × MASSSET 10/20/30), `_mloads` (trim + elevator-ramp
  transient). Plus `sample/cessna210_flagship_section_data.csv`.
- Model: the cessna210 stick + 324-box, 1000-spaced 5-surface mesh; `GRID 900` at the
  quarter-MAC **spliced into the fuselage CBAR chain** + z-down stability `CORD2R 990`
  (`AEROS` RCSID); `SPC1,1,1246,900` + `SUPORT,900,35`; 19 CONM2s + live MAT1 density for
  1081.0 kg at 22.0 % MAC (GPWG-checked); 4 × SPLINE2 (DTHY = 0.0) + 1 × **ATTACH** on the
  fin; part-chord 32-box elevator on the straight ⅔-chord line x = 7.60; MONPNT1 ×2 +
  MONPNT3 ×2; MASSSET FERRY/CRUISE/MTOW (996.0/1201.0/1560.0 kg, all three ops + an offset
  CONM2); MLOADS chain with the trimmed ELEV baked into TABLED1.
- Corrections **pre-baked and committed** with the viewer provenance header, generated by
  `build_from_section_data_multi` from a **3D-informed** CSV and converged by two-pass
  iteration on the reference incidence (ref α = trimmed α = 2.3904°).
- `docs/20_theory/02_realistic_airplane_sol144.md` + a `docs/00_INDEX.md` row.
- Staged deletions: `airplane_aero.bdf` (mesh-sizing rationale moved into the flagship
  header), `cessna210_aero.bdf`, `cessna210_section_data.csv`;
  `tests/aero/test_cessna210_example.py` re-pointed with **re-derived** numerics and
  `tests/aero/test_section_data.py` updated.

**Key decisions.**
- **Section data must be 3D-informed.** A VLM strip already carries the finite-wing
  downwash (installed slope 0.086 → 0.047 /deg on the wing; 0.044 /deg on the HTP, which
  also sits in the wing's downwash). Tabulating the 2D section slope (≈0.105 /deg) and
  letting WT2 enforce it double-counts the downwash and drives the wing alone to 6.0 /rad.
  The committed table is anchored to the model's own 3D strip loading, scaled ×1.0223 to
  the Helmbold AR-7.72 target (4.86 /rad), plus camber (a0 = −2.5° root), 3° washout
  (a0 → +0.5° tip), section `cm`, and an 8–14° reduced-slope region. Net effect: CL₀ goes
  0 → 0.099 while the slope moves only 5.21 → 5.33 /rad.
- **Stiffness calibrated, not inherited.** The pre-flagship PBAR left the wing effectively
  rigid (+0.44 % elastic, divergence 175× q). Recalibrated to I2 = 1.0e-5, J = 3.0e-5:
  **+3.5 % elastic-restrained CZ_α, divergence 23× q**, tip deflection 85 mm/1g and
  250 mm/2.5g (1.5 %/4.5 % semispan). Established empirically that for a wing CBAR along
  global Y with orientation vector (1,0,0) it is **I2** that resists flapwise bending, not
  I1 — the inherited deck's comment had them swapped.
- **Cross-section area is a mass property.** The inherited `A = 0.020` fuselage was 564 kg
  of structure at CG x = 4.06, which made the 22 % MAC target unreachable by any CONM2
  arrangement. Areas re-sized so ρAL lands on believable structural weights (349 kg total).
- **The fin gets ATTACH, not SPLINE2** — first `ATTACH` card in any `.bdf`/`.dat` in the
  repo, closing the gap found during DEF-M11. Chosen because a SPLINE2 there would need a
  spline axis along global Z, because the fin's own bending is negligible, and because at
  β = 0 it cannot perturb the trim gates. Directly exercises the DEF-M11 vertical-panel fix.
- **`MLDCOMD` tables are absolute ⇒ mass-case specific.** First coverage of MLOADS ×
  MASSSET showed the committed baseline table does not start at another mass case's trimmed
  elevator, so the run begins with a step input. Deck-authoring property, not a solver
  defect: re-authored for the case, the initial condition reproduces to 3e-16. Documented
  in `05c_sol144_maneuver.md` and gated by T6.
- **Doc defect fixed:** `02_card_reference.md` documented the `AESURF` `CID1` hinge as the
  **x**-axis; `integration.build_djx` takes `R[:, 1]`, the **y**-axis. Following the docs
  produced an identically-zero ELEV column and a bare `LinAlgError: Matrix is singular`
  from the trim Schur solve.

**Test/Acceptance.** `tests/aero/test_cessna210_flagship.py` — 24 checks, 28 s:
T1 three drivers parse via INCLUDE, 324 boxes, no box-ID collision, every box splined, zero
warnings, GPWG 1081 kg @ 22 % MAC; T2 correction regenerates from the CSV (tolerance set by
the `%.6E` card format, DEF-M12) and the table is 3D-informed not 2D; T3 both trims close on
n·W to 1e-6, CZ_α 5.2 ± 0.5, elastic shift in a 2–6 % band, elevator −3.91 °/g; T3b both
trimmed incidences inside the baked −4…8° region; T4 mass sweep — ANGLEA strictly monotone
and **wing-root bending per unit weight falls 0.760 → 0.635 → 0.586** (inertia relief, not
covered by any other test), MONPNT1 = n·W and MONPNT3 ≈ 0; T5 t=0 equals the trim, a
command held at trim moves nothing (3.3e-14), the ramp tracks and settles; T6 MLOADS ×
MASSSET both halves; T7 fin ATTACH resolves from file and transfers side force exactly
