...
| `sbeam/assembly/reduction.py` | **New (refactor, shared with `matrix_gaf_export`).** `reduce_to_aset(...)` extracted from `sol144._build_qaa_aset` (`:69`) and `_compute_aset_data` (`:355`); behaviour-preserving, gated by the SOL 144 + SOL 103 suites passing bit-identical. Extract **once**; both this design and the exporter consume it. |
